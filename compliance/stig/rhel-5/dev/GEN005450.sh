...
# Rule ID: SV-37811r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN005450
# Rule Title: The system must use a remote syslog server (loghost).
#
# Vulnerability Discussion: A syslog server (loghost) receives syslog 
# messages from one or more systems.  This data can be used as an 
# authoritative log source in the event a system is compromised and its 
...
# Fix Text: 
#
# Edit the syslog configuration file and add an appropriate remote syslog 
# server.     
#######################DISA INFORMATION##################################
