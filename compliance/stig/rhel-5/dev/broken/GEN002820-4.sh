...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29252
# Group Title: GEN002820-4-4
# Rule ID: SV-37668r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-4-4
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S chown32" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S chown32" $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
