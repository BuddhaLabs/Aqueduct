...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29251
# Group Title: GEN002820-3
# Rule ID: SV-37667r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-3
...
# If "-S fchmodat" is not in the result, this is a finding.
#
# Fix Text: 
#
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S fchmodat " $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S fchmodat " $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-3#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S fchmodat " >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-3#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S fchmodat " >> $AUDITFILE
	    service auditd restart
	fi
