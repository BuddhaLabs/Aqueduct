...
AUDITCOUNT1=$( grep -c -e "-a exit,always -F arch=b64 -S init_module " $AUDITFILE )
AUDITCOUNT2=$( grep -c -e "-a exit,always -F arch=b32 -S init_module " $AUDITFILE )
...
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    ...
	    echo "-a exit,always -F arch=b64 -S init_module " >> $AUDITFILE
	    service auditd restart
	fi
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002825#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S init_module " >> $AUDITFILE
	    service auditd restart
	fi
