...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29242
# Group Title: GEN002760-3
# Rule ID: SV-37658r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002760-3
...
# If the result does not contain "-S adjtime", this is a finding.
#
# Fix Text: 
#
...
# Procedure:
# Add the following lines to the audit.rules file to enable auditing of 
# administrative, privileged, and security actions:
...
AUDITCOUNT641=$( grep -c -e "-a exit,always -F arch=b64 -S adjtimex" $AUDITFILE )
AUDITCOUNT321=$( grep -c -e "-a exit,always -F arch=b32 -S adjtimex" $AUDITFILE )	
# Start-Lockdown


#DISA got it wrong, yet again.  It's adjtimex...NOT adjtime.  
...
if [ $UNAME == $BIT64 ]
  then
	if [ $AUDITCOUNT641 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-3#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S adjtimex" >> $AUDITFILE
	    service auditd restart
	fi
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-3#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S adjtimex" >> $AUDITFILE
	    service auditd restart
	fi
