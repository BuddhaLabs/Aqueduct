...
# |          |   Creation            |                    |            |
# |__________|_______________________|____________________|____________|
# |    1.1   |   Only run if GUI is  |    Jem Jensen      | 9-apr-2013 |
# |          |   installed           |                    |            |
# |__________|_______________________|____________________|____________|
#
...
# Confirm that we actually have GConf2 installed.  Both the directory
# holding the files for this GEN as well as the GConf2 RPM must be missing
# in order for this check to stop execution of the script
if [ ! -f /usr/bin/gconftool-2 ] && ! rpm -q GConf2 >/dev/null 2>&1 ; then
  exit 0
fi
