...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29245
# Group Title: GEN002760-6
# Rule ID: SV-37661r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002760-6
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S clock_settime" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S clock_settime" $AUDITFILE )	
...
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-6#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S clock_settime" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-6#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S clock_settime" >> $AUDITFILE
	    service auditd restart
	fi
