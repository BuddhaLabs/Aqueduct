...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29253
# Group Title: GEN002820-5
# Rule ID: SV-37669r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-5
...
# Additionally, the following rule is required in systems supporting the 
# 32-bit syscall table (such as i686 and x86_64):
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S fchown32" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S fchown32" $AUDITFILE )
AUDITCOUNTCHOWN32=$( grep -c -e "-a exit,always -F arch=b32 -S fchown" $AUDITFILE )
AUDITCOUNTCHOWN64=$( grep -c -e "-a exit,always -F arch=b32 -S fchown" $AUDITFILE )
...
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-5#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S fchown32" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-5#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S fchown32" >> $AUDITFILE
	    service auditd restart
	fi
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-5#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S fchown" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNTCHOWN64 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-5#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S fchown" >> $AUDITFILE
	    service auditd restart
	fi
else
