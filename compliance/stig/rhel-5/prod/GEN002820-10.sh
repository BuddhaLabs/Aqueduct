...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29272
# Group Title: GEN002820-10
# Rule ID: SV-37716r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-10
...
# If "-S fsetxattr" is not in the result, this is a finding.
#
# Fix Text: 
#
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S fsetxattr " $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S fsetxattr " $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-10#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S fsetxattr " >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-10#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S fsetxattr " >> $AUDITFILE
	    service auditd restart
	fi
