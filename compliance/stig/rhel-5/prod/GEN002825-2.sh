...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29281
# Group Title: GEN002825-2
# Rule ID: SV-37728r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002825-2
# Rule Title: The audit system must be configured to audit the loading 
# and unloading of dynamic kernel modules - delete_module.
#
...
# Check Content:
#
# Determine if the delete_module syscall is audited.
...
AUDITCOUNT1=$( grep -c -e "-a exit,always -F arch=b64 -S delete_module " $AUDITFILE )
AUDITCOUNT2=$( grep -c -e "-a exit,always -F arch=b32 -S delete_module " $AUDITFILE )
AUDITCOUNT3=$( grep -c -e "-a exit,always -S init_module " $AUDITFILE )
# Start-Lockdown
...
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    ...
	    echo "-a exit,always -F arch=b64 -S delete_module " >> $AUDITFILE
	    service auditd restart
	fi
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002825#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S delete_module " >> $AUDITFILE
	    service auditd restart
	fi
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002825#############" >> $AUDITFILE
	    echo "-a exit,always -S delete_module " >> $AUDITFILE
	    service auditd restart
	fi
fi
