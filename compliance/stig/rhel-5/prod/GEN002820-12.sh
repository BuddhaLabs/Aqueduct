...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29275
# Group Title: GEN002820-12
# Rule ID: SV-37719r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-12
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S lremovexattr" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S lremovexattr" $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-12#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S lremovexattr" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-12#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S lremovexattr" >> $AUDITFILE
	    service auditd restart
	fi
