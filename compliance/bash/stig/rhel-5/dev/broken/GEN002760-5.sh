...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29244
# Group Title: GEN002760-5
# Rule ID: SV-37660r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002760-5
...
# If the result does not contain "-S stime", this is a finding.
#
# Fix Text: 
#
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S stime" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S stime" $AUDITFILE )	
...
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-5#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S stime" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-5#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S stime" >> $AUDITFILE
	    service auditd restart
	fi
