...
# |          |   Creation            |                    |            |
# |__________|_______________________|____________________|____________|
# |    1.1   |   Reduced errors      |   Jem Jensen       | 9-apr-2013 |
# |__________|_______________________|____________________|____________|
#	                                                                  
...
if [ $? != 0 ]
then
  # If inn isn't installed, check for and remove
  # a user named "news" if it exists
  grep '^news:' /etc/passwd > /dev/null
  if [ $? == 0 ]
  then
    userdel -r news > /dev/null
  fi 
  ...
  # If the syslog.conf file exists remove the "news" lines
