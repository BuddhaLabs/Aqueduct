...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29249
# Group Title: GEN002760-10
# Rule ID: SV-37665r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002760-10
...
# when the sched_setscheduler and set_setscheduler are audited. This not only 
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S sched_setscheduler" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S sched_setscheduler" $AUDITFILE )	
...
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-10#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S sched_setscheduler" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-10#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S sched_setscheduler" >> $AUDITFILE
	    service auditd restart
	fi
