...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29259
# Group Title: GEN002820-8
# Rule ID: SV-37677r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-8
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S setxattr" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S setxattr" $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-8#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S setxattr" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-8#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S setxattr" >> $AUDITFILE
	    service auditd restart
	fi
