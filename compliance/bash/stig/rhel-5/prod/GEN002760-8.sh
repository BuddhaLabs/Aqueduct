...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29247
# Group Title: GEN002760-8
# Rule ID: SV-37663r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002760-8
...
AUDITCOUNT642=$( grep -c -e "-a always,exit -F arch=b64 -S setdomainname" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a always,exit -F arch=b32 -S setdomainname" $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-8#############" >> $AUDITFILE
	    echo "-a always,exit -F arch=b64 -S setdomainname" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-8#############" >> $AUDITFILE
	    echo "-a always,exit -F arch=b32 -S setdomainname" >> $AUDITFILE
	    service auditd restart
	fi
