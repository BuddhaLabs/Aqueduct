...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29246
# Group Title: GEN002760-7
# Rule ID: SV-37662r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002760-7
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S sethostname" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S sethostname" $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-7#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S sethostname" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-7#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S sethostname" >> $AUDITFILE
	    service auditd restart
	fi
