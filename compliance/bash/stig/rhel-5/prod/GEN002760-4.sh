...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29243
# Group Title: GEN002760-4
# Rule ID: SV-37659r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002760-4
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S settimeofday" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S settimeofday" $AUDITFILE )	
...
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-4#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S settimeofday" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002760-4#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S settimeofday" >> $AUDITFILE
	    service auditd restart
	fi
