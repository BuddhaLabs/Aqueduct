...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29250
# Group Title: GEN002820-2
# Rule ID: SV-37666r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-2
...
# If "-S fchmod" is not in the result, this is a finding
#
# Fix Text: 
#
...
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-2#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S fchmod" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-2#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S fchmod" >> $AUDITFILE
	    service auditd restart
	fi
