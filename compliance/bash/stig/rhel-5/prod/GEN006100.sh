...
# Rule ID: SV-37871r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN006100
# Rule Title: The /etc/smb.conf file must be owned by root.
#
# Vulnerability Discussion: The /etc/smb.conf file allows access to other 
# machines on the network and grants permissions to certain users.  If it 
# is owned by another user, the file may be maliciously modified and the 
