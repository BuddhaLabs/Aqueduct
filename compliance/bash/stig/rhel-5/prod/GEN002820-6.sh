...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29255
# Group Title: GEN002820-6
# Rule ID: SV-37671r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-6
...
# If "-S fchownat" is not in the result, this is a finding.
#
# Fix Text: 
#
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S fchownat" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S fchownat" $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-6#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S fchownat" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-6#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S fchownat" >> $AUDITFILE
	    service auditd restart
	fi
