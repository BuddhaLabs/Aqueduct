...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29257
# Group Title: GEN002820-7
# Rule ID: SV-37673r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-7
...
#	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-7#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S lchown32 " >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNTLCHOWN -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-7#############" >> $AUDITFILE
	    echo "-a exit,always -S lchown " >> $AUDITFILE
	    service auditd restart
	fi
