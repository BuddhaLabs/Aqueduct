...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S chmod" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S chmod" $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    ...
	    echo "-a exit,always -F arch=b64 -S chmod" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S chmod" >> $AUDITFILE
	    service auditd restart
	fi
