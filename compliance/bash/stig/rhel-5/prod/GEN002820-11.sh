...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29274
# Group Title: GEN002820-11
# Rule ID: SV-37718r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-11
...
# If "-S removexattr" is not in the result, this is a finding.
#
# Fix Text: 
#
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S removexattr " $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S removexattr " $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-11#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S removexattr " >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-11#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S removexattr " >> $AUDITFILE
	    service auditd restart
	fi
