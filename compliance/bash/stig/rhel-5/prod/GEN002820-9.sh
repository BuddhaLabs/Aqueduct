...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-29261
# Group Title: GEN002820-9
# Rule ID: SV-37681r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN002820-9
...
# If "-S lsetxattr" is not in the result, this is a finding.
#
# Fix Text: 
#
...
AUDITCOUNT642=$( grep -c -e "-a exit,always -F arch=b64 -S lsetxattr" $AUDITFILE )
AUDITCOUNT322=$( grep -c -e "-a exit,always -F arch=b32 -S lsetxattr" $AUDITFILE )	
# Start-Lockdown
if [ $UNAME == $BIT64 ]
  then
	  ...
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-9#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b64 -S lsetxattr" >> $AUDITFILE
	    service auditd restart
	fi
	if [ $AUDITCOUNT322 -eq 0 ]
	  then
	    echo " " >> $AUDITFILE
	    echo "#############GEN002820-9#############" >> $AUDITFILE
	    echo "-a exit,always -F arch=b32 -S lsetxattr" >> $AUDITFILE
	    service auditd restart
	fi
