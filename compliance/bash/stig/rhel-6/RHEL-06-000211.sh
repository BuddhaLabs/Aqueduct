...
#	
#######################DISA INFORMATION##################################
# Group ID (Vulid): RHEL-06-000211
# Group Title: SRG-OS-000129
#
   
# Rule ID: RHEL-06-000211_rule
# Severity: high
# Rule Version (STIG-ID): RHEL-06-000211
# Rule Title: The telnet daemon must not be running.
#
# Vulnerability Discussion: The telnet protocol uses unencrypted network 
# communication, which means that data from the login session, including 
# passwords and all other information transmitted during the session, can 
# be stolen by eavesdroppers on the network. The telnet protocol is also 
...
# Output should indicate the "telnet" service has either not been 
...
# Run the following command to verify "telnet" is disabled through current 
